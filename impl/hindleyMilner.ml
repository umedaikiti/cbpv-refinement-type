...
let rec value_infer ctx = function
  | Term.TmVar id -> Type.Substitution.empty, Map.find_exn ctx id
...
and computation_infer ctx = function
  ...
  | SeqComp (c1, id, ty, c2) ->
      ...
      let m2, ty2 = computation_infer (Map.add_exn ctx id ty) c2 in
  ...
  | Force (v, ty) ->
      ...
      | None -> assert false)
  | Lambda (id, ty, c) ->
      let m, ty' = computation_infer (Map.add_exn ctx id ty) c in
  ...
  | PatternMatch (v, x, a, y, b, c) ->
      let mv, tyv = value_infer ctx v in
      let mc, tyc = computation_infer (Map.add_exn (Map.add_exn ctx x a) y b) c in
  ...
  | Case (v, x, a, m, y, b, n) ->
      let mv, tyv = value_infer ctx v in
      let mm, tym = computation_infer (Map.add_exn ctx x a) m in
      let mn, tyn = computation_infer (Map.add_exn ctx y b) n in
